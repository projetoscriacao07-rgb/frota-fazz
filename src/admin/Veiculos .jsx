  ...
  useEffect(() => {
    ...
    async function restaurar() {
      try {
        ...
        // Pode haver registros antigos que ficaram presos por outro motivo
        // (ex: de antes dessa correção existir). Só reabre a tela se o
        // veículo daquele registro ainda estiver, no banco, marcado como em
        // uso por essa mesma pessoa — senão fica reabrindo pra um veículo
        // que ela já nem está usando mais.
        let registro = null
        let veiculo = null
        for (const d of snap.docs) {
          const v = veiculos.find((vv) => vv.id === d.data().veiculoId)
          if (v && v.emUsoPor === colaboradorId) {
            registro = d
            veiculo = v
            break
          }
